                  ...
                  <div className="markdown-body text-slate-200 space-y-3">
                    <ReactMarkdown
                      ...
                      components={{
                        h1: ({ node, ...props }) => (
                          <h1
                            className="text-base sm:text-lg font-bold text-amber-400 border-b border-slate-800 pb-1 mt-3 mb-2"
                            {...props}
                          />
                        ),
                        h2: ({ node, ...props }) => (
                          <h2
                            className="text-sm sm:text-base font-bold text-amber-400/90 mt-3 mb-1.5"
                            {...props}
                          />
                        ),
                        h3: ({ node, ...props }) => (
                          <h3
                            className="text-xs sm:text-sm font-semibold text-amber-300 mt-2 mb-1"
                            {...props}
                          />
                        ),
                        p: ({ node, ...props }) => (
                          <p
                            className="text-xs sm:text-sm leading-relaxed text-slate-300 my-1"
                            {...props}
                          />
                        ),
                        ul: ({ node, ...props }) => (
                          <ul
                            className="list-disc list-inside space-y-1 my-2 pl-1 text-slate-300"
                            {...props}
                          />
                        ),
                        ol: ({ node, ...props }) => (
                          <ol
                            className="list-decimal list-inside space-y-1 my-2 pl-1 text-slate-300"
                            {...props}
                          />
                        ),
                        li: ({ node, ...props }) => (
                        ...
                        code: ({ node, inline, className, children, ...props }) => {
                          return inline ? (
                            <code
                              className="bg-slate-950 text-amber-300 border border-slate-800 px-1.5 py-0.5 rounded text-[11px] font-mono"
                              {...props}
                            >
                              {children}
                            </code>
                          ) : (
                            <pre className="bg-slate-950 border border-slate-800 p-3 rounded-xl overflow-x-auto my-2 text-[11px] sm:text-xs font-mono text-slate-200">
                        ...
                        blockquote: ({ node, ...props }) => (
                          <blockquote
                            className="border-l-2 border-amber-500/50 pl-3 py-1 italic bg-amber-500/5 my-2 rounded-r-lg text-slate-400"
                            {...props}
                          />
                        ),
                        table: ({ node, ...props }) => (
                          <div className="overflow-x-auto my-3">
                            <table
                              className="w-full text-left border-collapse border border-slate-800 text-xs"
                              {...props}
                            />
                          </div>
                        ),
                        th: ({ node, ...props }) => (
                          <th
                            className="bg-slate-950 border border-slate-800 p-2 font-semibold text-amber-400"
                            {...props}
                          />
                        ),
                        td: ({ node, ...props }) => (
