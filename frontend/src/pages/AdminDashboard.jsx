...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setTasks(Array.isArray(tasksRes.data.data) ? tasksRes.data.data : tasksRes.data?.tasks || []);
  ...
  const addModule = () => {
    setCourseForm({
      ...courseForm,
      modules: [...courseForm.modules, { moduleTitle: "", lessons: [] }],
    });
  };

  const removeModule = (mIndex) => {
    const updatedModules = courseForm.modules.filter((_, index) => index !== mIndex);
    setCourseForm({ ...courseForm, modules: updatedModules });
  };

  const handleModuleTitleChange = (mIndex, title) => {
    const updatedModules = [...courseForm.modules];
    updatedModules[mIndex].moduleTitle = title;
    setCourseForm({ ...courseForm, modules: updatedModules });
  };

  // Dynamic Lessons Management
  const addLesson = (mIndex) => {
    const updatedModules = [...courseForm.modules];
    updatedModules[mIndex].lessons.push({
      title: "",
      cloudinaryUrl: "",
      duration: "",
      type: "video",
  ...
  const removeLesson = (mIndex, lIndex) => {
    const updatedModules = [...courseForm.modules];
    updatedModules[mIndex].lessons = updatedModules[mIndex].lessons.filter(
      (_, index) => index !== lIndex
    );
    setCourseForm({ ...courseForm, modules: updatedModules });
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-10 space-y-8 bg-slate-50 min-h-screen text-slate-800">
                    ...
                    >
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-3 text-[10px] text-slate-400 pt-1">
                          ...
                          <span className="flex items-center gap-1">
                            <Users className="w-3 h-3 text-emerald-600" /> Assigned: {task.assignedTo || "Admin"}
                          </span>
                        </div>
                      </div>
                      <button
      ...
      {showCourseModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-slate-200 w-full max-w-2xl rounded-2xl p-6 space-y-4 shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
            ...
            <form onSubmit={handleCreateCourse} className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    required
                    type="text"
                    placeholder="e.g. Classical Arabic (Fusha)"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-slate-600 font-semibold mb-1">Course Image URL</label>
                  <input
                    type="url"
              ...
              <div className="space-y-3 pt-2 border-t border-slate-200">
                <div className="flex justify-between items-center">
                  <h4 className="font-bold text-slate-800 text-sm">Course Curriculum (Modules)</h4>
                  <button
                    type="button"
                    onClick={addModule}
                    className="px-3 py-1.5 bg-emerald-50 border border-emerald-200 text-emerald-700 hover:bg-emerald-100 font-bold rounded-lg flex items-center gap-1 transition"
                  >
                ...
                {courseForm.modules.map((module, mIndex) => (
                  <div key={mIndex} className="p-3 border border-slate-200 rounded-xl bg-slate-50/50 space-y-3">
                    <div className="flex items-center gap-2">
                      <input
                        required
                        type="text"
                        placeholder={`Module ${mIndex + 1} Title (e.g. Arabic Alphabets)`}
                        value={module.moduleTitle}
                        onChange={(e) => handleModuleTitleChange(mIndex, e.target.value)}
                        className="w-full bg-white border border-slate-200 rounded-lg p-2 text-slate-800 font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
                      />
                      <button
                        type="button"
                        onClick={() => removeModule(mIndex)}
                        className="p-2 text-rose-500 hover:text-rose-700 hover:bg-rose-50 rounded-lg shrink-0"
                      >
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>

                    {/* Nested Lessons Section */}
                    <div className="pl-3 border-l-2 border-emerald-500/30 space-y-2">
                      <div className="flex justify-between items-center">
                        <span className="text-[11px] font-bold text-slate-600">Lessons</span>
                        <button
                          type="button"
                      ...
                      {module.lessons.map((lesson, lIndex) => (
                        <div key={lIndex} className="p-2.5 bg-white border border-slate-200 rounded-lg space-y-2">
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            ...
                            <input
                              ...
                              value={lesson.cloudinaryUrl}
                              onChange={(e) => handleLessonChange(mIndex, lIndex, "cloudinaryUrl", e.target.value)}
                              className="bg-slate-50 border border-slate-200 rounded-lg p-2 focus:outline-none"
                            />
                          </div>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 items-center">
                            ...
                            <button
                              ...
                              className="text-rose-500 hover:text-rose-700 font-semibold text-[11px] text-right pr-1"
                            >
                              Remove Lesson
                            </button>
                          </div>
              ...
              <div className="flex justify-end gap-2 pt-3 border-t border-slate-100">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl flex items-center gap-2 disabled:opacity-50 cursor-pointer"
                >
