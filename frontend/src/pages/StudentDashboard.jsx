  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8 bg-slate-50 min-h-screen text-slate-800 font-sans">
      ...
      {upcomingLiveClass && isClassActive(upcomingLiveClass) && (
        <div className="bg-gradient-to-r from-emerald-50 via-white to-amber-50/60 border border-emerald-200/80 rounded-3xl p-5 sm:p-6 relative overflow-hidden shadow-lg shadow-slate-200/50 transition-all">
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-6 relative z-10">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 flex-1">
              ...
              {/* Class Thumbnail Image / Fallback Box */}
              <div className="w-full sm:w-32 h-28 sm:h-24 rounded-2xl overflow-hidden bg-slate-100 border border-slate-200 shrink-0 relative">
                {upcomingLiveClass.thumbnail || upcomingLiveClass.bannerImage || upcomingLiveClass.course?.image ? (
                  <img
                    src={upcomingLiveClass.thumbnail || upcomingLiveClass.bannerImage || upcomingLiveClass.course?.image}
                    alt={upcomingLiveClass.title}
                    className="w-full h-full object-cover"
                  />
                ) : (
                  <div className="w-full h-full bg-gradient-to-br from-emerald-100 via-amber-50 to-emerald-200 flex flex-col items-center justify-center p-2 text-center">
                    <Sparkles className="w-6 h-6 text-emerald-600 mb-1 opacity-80" />
                    <span className="text-[10px] font-bold text-slate-700 line-clamp-1">
                      {upcomingLiveClass.category || "Live Class"}
                    </span>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2 flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="inline-flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-red-100 border border-red-200 text-red-600 text-[11px] font-bold animate-pulse">
                    <span className="w-2 h-2 rounded-full bg-red-500 animate-ping" />
                    {upcomingLiveClass.status === "Live" ? "LIVE NOW" : "Live Session Scheduled"}
                  </span>

                  {upcomingLiveClass.category && (
                    <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-amber-800 bg-amber-100/80 border border-amber-200 px-2 py-0.5 rounded-md">
                ...
                <p className="text-xs text-slate-600 line-clamp-1">
                  ...
                  <span className="text-slate-900 font-semibold">
                    {upcomingLiveClass.course?.title || upcomingLiveClass.courseName || "General Session"}
                  </span>{" "}
                  • Instructor:{" "}
                  <span className="text-slate-800 font-medium">
                    {upcomingLiveClass.scholarName || upcomingLiveClass.instructor || "Guest Scholar"}
                  </span>
                </p>

                <div className="flex flex-wrap items-center gap-4 text-xs text-emerald-700 font-semibold pt-0.5">
                  <span className="flex items-center gap-1.5">
                    <Calendar className="w-3.5 h-3.5 text-emerald-600" />
                    {upcomingLiveClass.scheduledAt
                      ? new Date(upcomingLiveClass.scheduledAt).toLocaleDateString([], {
                        weekday: "short",
                        month: "short",
                        day: "numeric",
                      })
                      : upcomingLiveClass.date}
                  </span>
                  <span className="flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5 text-amber-600" />
                    {upcomingLiveClass.scheduledAt
                      ? new Date(upcomingLiveClass.scheduledAt).toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit",
                      })
            ...
            <div className="w-full md:w-auto shrink-0 flex items-center">
              <a
                href={upcomingLiveClass.meetingUrl || upcomingLiveClass.meetingLink || "#"}
                target="_blank"
                rel="noopener noreferrer"
                className="w-full md:w-auto px-6 py-3.5 rounded-2xl bg-gradient-to-r from-emerald-600 to-emerald-700 hover:from-emerald-700 hover:to-emerald-800 text-white font-extrabold text-xs transition flex items-center justify-center gap-2 shadow-md shadow-emerald-600/20 active:scale-95"
              >
